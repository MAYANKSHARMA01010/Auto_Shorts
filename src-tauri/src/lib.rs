...
async fn environment_status(state: tauri::State<'_, AppState>) -> Result<EnvironmentStatus, String> {
    ...
    Ok(EnvironmentStatus {
        ...
        has_deepgram_key: env_key_set("DEEPGRAM_API_KEY"),
        has_anthropic_key: env_key_set("ANTHROPIC_API_KEY"),
        has_deepseek_key: env_key_set("DEEPSEEK_API_KEY"),
        has_gemini_key: env_key_set("GEMINI_API_KEY"),
        has_openai_key: env_key_set("OPENAI_API_KEY"),
        has_openrouter_key: env_key_set("OPENROUTER_API_KEY"),
        llm_provider,
...
async fn transcribe_project(
    ...
    let duration_sec = media::probe_media(&project.source_path)
        .ok()
    ...
    let transcript = match provider.as_str() {
        "deepgram" => {
            ...
            emit_progress("Calling Deepgram API", 100.0);
        ...
        "local" => {
            ...
            let transcribe_progress = {
                let emit = emit_progress.clone();
                Box::new(move |p: f64| emit("Transcribing", p)) as Box<dyn Fn(f64) + Send>
            };

            emit_progress("Transcribing", 0.0);

            transcription::transcribe_local_with_progress(
    ...
    // ── Spelling Fix Step (LLM post-processing) ──────────────────────────────
    // Only run for local transcription and when an LLM is configured.
    // Timestamps are NEVER changed — only word text is corrected.
    let mut transcript = transcript;
    if provider == "local" {
        if let Some(ref lp) = llm_provider {
            if lp != "local" || llm_model_name.is_some() {
                emit_progress("Fixing Spelling", 0.0);
                let fix_result = llm::fix_spelling(
                    &mut transcript.words,
                    lp.as_str(),
                ...
                if let Err(e) = fix_result {
                    // Non-fatal — log but continue with uncorrected words
                    eprintln!("[spelling-fix] Skipped due to error: {e}");
                } else {
                    // Rebuild segments text from corrected words
                    transcript.segments = transcription::build_segments(&transcript.words);
                }
                emit_progress("Fixing Spelling", 100.0);
            }
        }
    }
...
#[tauri::command]
async fn import_custom_transcript(
    state: tauri::State<'_, AppState>,
    project_id: String,
    vtt_content: String,
) -> Result<Transcript, String> {
...
async fn generate_candidates(
    ...
    for p in try_list {
        let result = match p.as_str() {
            "claude" => {
                ...
                if let Some(key) = key {
                    Some(llm::detect_candidates_with_claude(&normalized, &key).await)
                } else {
                    None
                }
            }
            "deepseek" => {
                let key = api_keys.as_ref().and_then(|keys| keys.get("deepseek").cloned())
                    ...
                    .filter(|v| !v.trim().is_empty());
                if let Some(key) = key {
                    Some(llm::detect_candidates_with_deepseek(&normalized, &key).await)
                } else {
                    None
                }
            }
            "gemini" => {
                let key = api_keys.as_ref().and_then(|keys| keys.get("gemini").cloned())
                    ...
                    .filter(|v| !v.trim().is_empty());
                if let Some(key) = key {
                    Some(llm::detect_candidates_with_gemini(&normalized, &key).await)
                } else {
                    None
                }
            }
            "openai" => {
                let key = api_keys.as_ref().and_then(|keys| keys.get("openai").cloned())
                    ...
                    .filter(|v| !v.trim().is_empty());
                if let Some(key) = key {
                    Some(llm::detect_candidates_with_openai(&normalized, &key).await)
                } else {
                    None
                }
            }
            "openrouter" => {
                let key = api_keys.as_ref().and_then(|keys| keys.get("openrouter").cloned())
                    ...
                    .filter(|v| !v.trim().is_empty());
                if let Some(key) = key {
                    Some(llm::detect_candidates_with_openrouter(&normalized, &key).await)
                } else {
                    None
                }
            }
            "local" | "ollama" => {
                let model = if p == active_provider && model_name.is_some() {
                    model_name.clone().unwrap()
                } else {
                    std::env::var("OLLAMA_MODEL").unwrap_or_else(|_| "qwen2.5".to_string())
                };
                Some(llm::detect_candidates_with_local_llm(&normalized, &model).await)
            }
            _ => None,
        };
        ...
        if let Some(res) = result {
            match res {
                Ok(d) => {
                    if d.is_empty() {
                        println!("Provider '{}' returned 0 candidates. Trying next...", p);
                        errors.push(format!("{}: Returned 0 candidates", p));
                    } else {
                        println!("Successfully generated candidates using {}!", p);
                        drafts_opt = Some(d);
                        break;
                    }
                }
                Err(e) => {
                    println!("Provider '{}' failed: {}", p, e);
                    errors.push(format!("{}: {}", p, e));
                }
            }
        } else if p == active_provider {
            errors.push(format!("{}: No API key provided", p));
        }
    }
    ...
    let candidates = db
        .add_candidates(&project_id, &drafts)
        .map_err(to_command_error)?;
...
fn render_flat_clip_for_candidate(
    ...
    let mut render_warning: Option<String> = None;
    ...
    if let Ok(Some(transcript_record)) = state.db.latest_transcript(&project.id) {
        if let Ok(normalized) = serde_json::from_str::<NormalizedTranscript>(&transcript_record.raw_json) {
            ...
            if media::drawtext_supported() {
                ...
                render_warning = Some("Your FFmpeg installation is missing libfreetype (drawtext filter). The clip was cut, but captions could not be burned in. To fix this, please run: brew reinstall ffmpeg".to_string());
            }
        }
    }
...
#[tauri::command]
async fn generate_candidate_metadata_cmd(
    state: tauri::State<'_, AppState>,
    candidate_id: String,
    provider: String,
    api_key: Option<String>,
    ...
    // Fetch transcript text for this candidate
    let transcript = db
    ...
    for seg in &candidate.segments {
        for word in &normalized.words {
            ...
            if mid >= seg.start && mid <= seg.end {
                candidate_text.push_str(&word.text);
                candidate_text.push(' ');
            }
        }
    }
    ...
    let active_key = match provider.as_str() {
        ...
        "gemini" => api_key.or_else(|| std::env::var("GEMINI_API_KEY").ok()),
        "openai" => api_key.or_else(|| std::env::var("OPENAI_API_KEY").ok()),
        "openrouter" => api_key.or_else(|| std::env::var("OPENROUTER_API_KEY").ok()),
...
pub fn run() {
    ...
    tauri::Builder::default()
        ...
        .invoke_handler(tauri::generate_handler![
            ...
            open_path,
            update_transcript,
            generate_candidate_metadata_cmd,
            review_candidate_cmd,
            predict_viral_cmd,
            import_custom_transcript
        ])
